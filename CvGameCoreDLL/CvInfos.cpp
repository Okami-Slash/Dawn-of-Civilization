...
m_piLandYield(NULL), // 1SDAN
m_piWaterYield(NULL), // 1SDAN
...
CvCivicInfo::~CvCivicInfo()
{
	...
	SAFE_DELETE_ARRAY(m_piLandYield); // 1SDAN
	SAFE_DELETE_ARRAY(m_piWaterYield); // 1SDAN
...
// 1SDAN
int CvCivicInfo::getCoreLuxuryHappiness() const
{
...
// 1SDAN
int CvCivicInfo::getLandYield(int i) const
{
	FAssertMsg(i < NUM_YIELD_TYPES, "Index out of bounds");
	FAssertMsg(i > -1, "Index out of bounds");
	return m_piLandYield ? m_piLandYield[i] : -1;
}

// 1SDAN
int* CvCivicInfo::getLandYieldArray() const
{
	return m_piLandYield;
}

// 1SDAN
int CvCivicInfo::getWaterYield(int i) const
{
	FAssertMsg(i < NUM_YIELD_TYPES, "Index out of bounds");
	FAssertMsg(i > -1, "Index out of bounds");
	return m_piWaterYield ? m_piWaterYield[i] : -1;
}

// 1SDAN
int* CvCivicInfo::getWaterYieldArray() const
{
	return m_piWaterYield;
}
...
void CvCivicInfo::read(FDataStreamBase* stream)
{
	...
	stream->Read(&m_iCoreLuxuryHappiness); // 1SDAN
	...
	// 1SDAN
	SAFE_DELETE_ARRAY(m_piLandYield);
	m_piLandYield = new int[NUM_YIELD_TYPES];
	stream->Read(NUM_YIELD_TYPES, m_piLandYield);

	// 1SDAN
	SAFE_DELETE_ARRAY(m_piWaterYield);
	m_piWaterYield = new int[NUM_YIELD_TYPES];
	stream->Read(NUM_YIELD_TYPES, m_piWaterYield);
...
void CvCivicInfo::write(FDataStreamBase* stream)
{
	...
	stream->Write(m_iCoreLuxuryHappiness); // 1SDAN
	...
	stream->Write(NUM_YIELD_TYPES, m_piLandYield); // 1SDAN
	stream->Write(NUM_YIELD_TYPES, m_piWaterYield); // 1SDAN
...
bool CvCivicInfo::read(CvXMLLoadUtility* pXML)
{
	...
	pXML->GetChildXmlValByName(&m_iCoreLuxuryHappiness, "iCoreLuxuryHappiness"); // Leoreth
	...
	// 1SDAN
	if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(), "LandYields"))
	{
		pXML->SetYields(&m_piLandYield);
		gDLL->getXMLIFace()->SetToParent(pXML->GetXML());
	}
	else
	{
		pXML->InitList(&m_piLandYield, NUM_YIELD_TYPES);
	}

	// 1SDAN
	if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(), "WaterYields"))
	{
		pXML->SetYields(&m_piWaterYield);
		gDLL->getXMLIFace()->SetToParent(pXML->GetXML());
	}
	else
	{
		pXML->InitList(&m_piWaterYield, NUM_YIELD_TYPES);
	}
