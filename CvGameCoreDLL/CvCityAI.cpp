			...
			if ((eIgnoreAdvisor == NO_ADVISOR) || (GC.getUnitInfo(eLoopUnit).getAdvisorType() != eIgnoreAdvisor))
			{
				if (!isHuman() || (GC.getUnitInfo(eLoopUnit).getDefaultUnitAIType() == eUnitAI))
				{
					if ((!(isFoodProduction(eLoopUnit) && (bGrowMore))) || 
						(((getOwner() == HARAPPA && eUnitAI == UNITAI_SETTLE) || (eUnitAI == UNITAI_WORKER && GET_PLAYER(getOwner()).AI_getNumAIUnits(UNITAI_WORKER) == 0)) && 
						(isCapital() && GC.getGame().getGameTurn() < ((20 * GC.getGameSpeedInfo(GC.getGameINLINE().getGameSpeedType()).getTrainPercent()) / 100))))
					{
						if (canTrain(eLoopUnit))
						{
